...
if(inputValue) {
    inputValue.addEventListener('input', FormatValue);
    inputValue.addEventListener('blur', FormatValueAfterBlur);
}
...
function FormatValue(e) {
    ...
    input = SanitizeInput(input, "0123456789");
    ...
    if (input.length > 2) {
        ...
        for (i = 5; i <= input.length; i += 3) {
            Result = input.substr(input.length - i, 3) + Result;
            if (input.length > i) Result = "." + Result;
        };

        Result = input.substr(0, input.length - i + 3) + Result;
    } else {
        Result = input;
    };
...
function SanitizeInput(input, filter) {
    ...
    for (i = 0; i < input.length; i++) {
        ...
        for (j = 0; j < filter.length; j++) {
            if (c == filter.substr(j, 1)) break;
        };
        
        if (j < filter.length) Result = Result + c;
    };

    return Result;
}

function FormatValueAfterBlur(e) {
